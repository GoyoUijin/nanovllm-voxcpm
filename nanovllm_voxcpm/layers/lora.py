...
class LoRAQKVParallelLinear(nn.Module):
    ...
    def _base_weight_loader(self, param: nn.Parameter, loaded_weight: torch.Tensor, loaded_shard_id: str = None):
...
class LoRAMergedColumnParallelLinear(nn.Module):
    ...
    def _base_weight_loader(self, param: nn.Parameter, loaded_weight: torch.Tensor, loaded_shard_id: int = None):
        """Base weight loader"""
        if loaded_shard_id is None:
            param.data.copy_(loaded_weight)
            return
...
def iter_lora_modules(model: nn.Module):
    ...
    for module in model.modules():
        if isinstance(module, (LoRAQKVParallelLinear, LoRAMergedColumnParallelLinear, LoRARowParallelLinear, LoRALinear)):
            if module.lora_r > 0:
                yield module
